...
export type HazardLayerItemProps = {
    hazard: Hazard;
};

type GeoExplorerRootState = ReturnType<typeof geoExplorerStore.getState>;

export const HazardLayerItem = ({ hazard }: HazardLayerItemProps) => {
    const dispatch = useDispatch();
    const mapLayers = useSelector((state: GeoExplorerRootState) => state.explore.mapLayers);

    const isSelected = mapLayers.some((layer) => layer.data.layer_id === hazard.id);

    const handleSelect = () => dispatch(selectDataset({ layer_id: hazard.id }));

    const handleAdd = () => dispatch(addLayer({ layer_id: hazard.id }));

    const handleRemove = () => dispatch(removeLayer({ layer_id: hazard.id }));
