...
import { store as geoExplorerStore } from '@ncsa/geo-explorer';
...
export function DatasetLayerItem({ dataset }: DatasetLayerItemProps) {
  ...
  const mapLayers = geoExplorerStore.getState().explore.mapLayers;

  const isSelected = mapLayers.some(
    (layer) => layer.data.layer_id === dataset.id,
  );

  const handleSelect = () =>
    dispatch(selectDataset({ layer_id: dataset.id }));

  const handleAdd = () =>
    dispatch(addLayer({ layer_id: dataset.id }));

  const handleRemove = () =>
    dispatch(removeLayer({ layer_id: dataset.id }));
